...
csvfile <- paste("/Users/yunpeng/data/forest_npp/NPP_final.csv")
write.csv(NPP_final3, csvfile, row.names = TRUE)
...
gg$ggmap + geom_point(data=subset(NPP2,wnf_obs_final>0 & pred_wnf>0 & pft2 == "Forest"),aes(lon,lat),col="red") 
gg$gglegend

summary(NPP2$CN_wood_final)


analyse_modobs2(subset(NPP2,wnf_obs_final>0 & pred_wnf>0),"pred_wnf","wnf_obs_final", type = "points")
...
cn_root_df$CN_root_final <- (CN_Schulz2$c_fineroot + CN_Schulz2$c_coarseroot) / CN_Schulz2$n_root
...
gg$ggmap + geom_point(data=subset(NPP2,bnf_obs_final>0 & pred_bnf>0 & pft2 == "Forest"),aes(lon,lat),col="red")
gg$gglegend
...
ggplot(subset(cn_root_df,pred_nuptake>0 & obs_nuptake>0 & pft2 == "Forest"), aes(x=pred_nuptake, y=obs_nuptake)) +
    geom_point()+geom_abline(intercept=0,slope=1)+geom_smooth(method = "lm", se = TRUE)+
    xlab("Prediction")+ylab("Observation")+theme_classic()
...
ggplot(subset(cn_root_df,pred_nuptake_gwr>0 & obs_nuptake>0 & pft2 == "Forest"), aes(x=pred_nuptake_gwr, y=obs_nuptake)) +
    geom_point()+geom_abline(intercept=0,slope=1)+geom_smooth(method = "lm", se = TRUE)+
    xlab("Prediction")+ylab("Observation")+theme_classic()
summary(lm(obs_nuptake~pred_nuptake_gwr,cn_root_df))

gg <- plot_map3(nuptake_df[,c(1,2,4)], 
                varnam = "nuptake",plot_title = " Total N uptake in ecosystem (gN/m2/yr)",
                latmin = -65, latmax = 85, combine = FALSE)
gg$ggmap + geom_point(data=subset(cn_root_df,obs_nuptake>0 & pred_nuptake_gwr>0 & pft2 == "Forest"),aes(lon,lat),col="red")
gg$gglegend
...
#accounting for three main parts 
summary(lnpp_df)
summary(leafcn_df)
summary(nre_df)

lnf_final <- cbind(lnpp_df,leafcn_df$leafcn,nre_df$nre)
names(lnf_final) <- c("lon","lat","z","lnpp","leafnc","nre")

lnf_final2 <- na.omit(lnf_final)

lnf_final3 <- subset(lnf_final2,lnpp>0)
summary(lnf_final3)

#holding each factor as constant
lnf_final3$lnf1_lnpp <- lnf_final3$lnpp*0.04440* (1-0.5464)
lnf_final3$lnf2_leafnc <- 127.105*lnf_final3$leafnc* (1-0.5464)
lnf_final3$lnf3_nre <- 127.105*0.04440* (1-lnf_final3$nre)

a1 <- lnf_final3$lnf1_lnpp
a2 <- lnf_final3$lnf2_leafnc
a3 <- lnf_final3$lnf3_nre

var(a1)/(var(a1)+var(a2)+var(a3))
var(a2)/(var(a1)+var(a2)+var(a3))
var(a3)/(var(a1)+var(a2)+var(a3))

results <- c(a1,a2,a3)
group <- c(rep("Leaf NPP: 95% of variance",length(a1)),rep("Leaf N/C : 2% of variance",length(a1)),rep("Nitrogen resorption efficiency: 3% of variance",length(a1)))

results_final <- as.data.frame(cbind(results,group))
results_final$results <- as.numeric(results_final$results)
ggplot(results_final, aes(x=results, fill=group)) + geom_density(alpha=.3)+xlab("Leaf N flux (gN/m2/yr)")+ggtitle("Leaf N flux = NPP.leaf * leaf N/C * (1-NRE)")+ labs(fill = "Variance of factor (when holding other 2 factors as constant)")+theme_classic()

#quanfitying each part
allocation <- cbind(gpp_df,npp_df[,4],anpp_df[,4],lnpp_df[,4],wnpp_df[,4],bnpp_df[,4])
names(allocation) <- c("lon","lat","z","gpp","npp","anpp","lnpp","wnpp","bnpp")
summary(allocation)
allocation <- na.omit(allocation)
allocation2 <- subset(allocation,npp>0 & lnpp>0) #excluding 0 value grids (set as 0 when in global simulations - prediction fields in land was empty)
summary(allocation2$npp/allocation2$gpp)
summary(allocation2$anpp/allocation2$npp)
summary(allocation2$bnpp/allocation2$npp)
summary(allocation2$lnpp/allocation2$anpp)
