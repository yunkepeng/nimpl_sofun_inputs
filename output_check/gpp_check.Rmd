...
#(1) first pred's gpp, from nimpl (gpp_df)
a <- 1.5
for (i in c(1:nrow(gpp_site))){
  ...
  gpp_site$nimpl_gpp[i] <- (gwr(gpp ~ z, gpp_part, bandwidth = 1.06, fit.points =gpp_coord,predictions=TRUE))$SDF$pred
}

#(2) second pred's gpp, from gmd_gpp (gpp_gmd)
for (i in c(1:nrow(gpp_site))){
  ...
  gpp_coord <- gpp_site[i,1:3]
  coordinates(gpp_coord) <- c("lon","lat")
  gpp_site$gmd_gpp[i] <- (gwr(gpp ~ z, gpp_part, bandwidth = 1.06, fit.points =gpp_coord,predictions=TRUE))$SDF$pred
}
...
analyse_modobs2(subset(gpp_forest,nimpl_gpp>0),"GPP","nimpl_gpp", type = "points")

######2. Forest GPP: gmd_gpp vs. obs. gpp: we did some some undestimation.
analyse_modobs2(subset(gpp_forest,gmd_gpp>0),"GPP","gmd_gpp", type = "points")

######3. Forest GPP: gmd_gpp vs. nimpl_gpp -> gmd slightly high, but not too much!
analyse_modobs2(subset(gpp_forest,gmd_gpp>0),"nimpl_gpp","gmd_gpp", type = "points")
