...
NPP_grassland_final10 <- NPP_grassland_final9[-c(2,6,8,9,10,14,21,22,23,24,28,29,40,41),]
dim(NPP_grassland_final10)


#anpp - simulated

NPP_grassland_final9$simulated_anpp <- NPP_grassland_final9$weightedgpp_all * 0.5 * (1/(1 + exp(-( -2.5821 + NPP_grassland_final9$Tg*0.0370 + NPP_grassland_final9$alpha*1.9772))))
analyse_modobs2(subset(NPP_grassland_final9,ANPP_2 >0 & simulated_anpp>0 &is.na(filter)==TRUE),"ANPP_2","simulated_anpp", type = "points")

#anpp - constant of anpp/gpp
NPP_grassland_final9$constant_anpp <- NPP_grassland_final9$weightedgpp_all *0.38
analyse_modobs2(subset(NPP_grassland_final9,ANPP_2 >0 & constant_anpp>0),"ANPP_2","constant_anpp", type = "points")

#npp - constant - not good
NPP_grassland_final9$constant_npp <- NPP_grassland_final9$weightedgpp_all*0.5
analyse_modobs2(subset(NPP_grassland_final9,TNPP_1 >0 & constant_npp>0),"TNPP_1","constant_npp", type = "points")

#bnpp
summary(lmer(log((BNPP_1/TNPP_1)/(1-(BNPP_1/TNPP_1)))~Tg+alpha+(1|site),
             data=subset(NPP_grassland_final9,BNPP_1 >0 &is.na(filter)==TRUE)))

#npp - simulated by by gpp - not good

final <- subset(NPP_grassland_final9, TNPP_1>0 & weightedgpp_all>0 & is.na(filter)==TRUE)
dim(final)
outliers <- subset(final,weightedgpp_all<TNPP_1)
dim(outliers)

hist(outliers$BNPP_1/outliers$TNPP_1)
hist(NPP_grassland_final9$BNPP_1/NPP_grassland_final9$TNPP_1)

summary(lmer((TNPP_1)~(weightedgpp_all)+(1|site),data=subset(NPP_grassland_final9, is.na(filter)==TRUE)))
#summary(lmer(log(TNPP_1)~log(pred_gpp_c4)+(1|site),data=NPP_grassland_final9))
NPP_grassland_final9$sumulated_npp <- 0.42745*(NPP_grassland_final9$weightedgpp_all)
analyse_modobs2(subset(NPP_grassland_final9, is.na(filter)==TRUE),"TNPP_1","sumulated_npp", type = "points")


#anpp based on statistical model
summary(lmer(log((ANPP_2/TNPP_1)/(1-(ANPP_2/TNPP_1)))~Tg+alpha+(1|site),
             data=subset(NPP_grassland_final9,ANPP_2 >0)))
NPP_grassland_final9$simulated_anpp_filtered <- NPP_grassland_final9$sumulated_npp_c4_filtered * (1/(1 + exp(-( -2.5821 + NPP_grassland_final9$Tg*0.0370 + NPP_grassland_final9$alpha*1.9772))))
analyse_modobs2(subset(NPP_grassland_final9,ANPP_2 >0 & simulated_anpp_filtered>0 &is.na(filter)==TRUE),"ANPP_2","simulated_anpp_filtered", type = "points")

aaa <- subset(NPP_grassland_final9,ANPP_2>1000)

#anpp based on npp
summary(lmer(log(ANPP_2)~log(TNPP_1)+(1|site),data=subset(NPP_grassland_final9,ANPP_2<TNPP_1 & is.na(filter)==TRUE)))
NPP_grassland_final9$sumulated_anpp_c4_filtered <- exp(0.88828 + 0.68272*log(NPP_grassland_final9$sumulated_npp_c4_filtered))
analyse_modobs2(subset(NPP_grassland_final9, is.na(filter)==TRUE),"ANPP_2","sumulated_anpp_c4_filtered", type = "points")


#weighted
summary(lmer(log(TNPP_1)~log(weightedgpp_all)+(1|site),data=subset(NPP_grassland_final9,TNPP_1<weightedgpp_all)))
summary(lmer(log(TNPP_1)~log(weightedgpp_all)+(1|site),data=NPP_grassland_final9))
NPP_grassland_final9$sumulated_npp <- exp(2.3653 + 0.5192*log(NPP_grassland_final9$weightedgpp_all))
analyse_modobs2(subset(NPP_grassland_final9,TNPP_1<weightedgpp_all),"TNPP_1","sumulated_npp", type = "points")
...
summary(lmer(log((ANPP_2/TNPP_1)/(1-(ANPP_2/TNPP_1)))~Tg+alpha+(1|site),data=NPP_grassland_final9))
r.squaredGLMM(lmer(log((ANPP_2/TNPP_1)/(1-(ANPP_2/TNPP_1)))~Tg+alpha+(1|site),data=NPP_grassland_final9))
...
summary(lm(GPP~weightedgpp_all,subset(NPP_grassland_final9,pft=="Grassland")))
