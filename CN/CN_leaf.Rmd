...
#Narea/LMA (Nmass,unitness) * C/N ratio (as shown firstly) = Cmass (unitness) 
...
#Now, input site-mean coef (Ns = 0.0144, Nr = 0.0050, as shown above)
...
location <- "~/data/output_leafcn/sitemean_coef/" # site-mean coefficients
...
#####Compare species-based prediction with species-based obs. data???
firstyr_data <- 2007 # In data file, which is the first year
endyr_data <- 2018 # In data file, which is the last year
location <- "~/data/output_leafcn/species_coef/" #species-based coefficients
...
#above is to apply the same like above, it still fitted less good. No worries! Let's see how our site-based Nmass prediction compared with our site-species based data?

leafcn_pred_direct2 <- leafcn_pred_direct[,c("lon","lat","z","nimpl_nmass_directly","nmass_gwr_pred")]

names(leafcn_pred_direct2) <- c("lon","lat","Elevation","nimpl_nmass_directly","nmass_gwr_pred")

#merge site-based prediction to site-species based obs. data
final_species_predict <- merge(leafcn_pred_direct2,sitespecies_final,by=c("lat","lon","Elevation"),all.y=TRUE) 
dim(sitespecies_final)
dim(final_species_predict)

summary(final_species_predict)

#using direct method
analyse_modobs2(final_species_predict,"nmass","nimpl_nmass_directly", type = "points")
#using gwr method
analyse_modobs2(final_species_predict,"nmass","nmass_gwr_pred", type = "points")
#ok...just ignore....
