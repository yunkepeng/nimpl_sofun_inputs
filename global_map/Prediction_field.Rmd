...
#1. soil C/N
The original stand age file was available at "~/data/nimpl_ISRIC/data_orig/", then after scripts below and after CDO processing (for correcting coordinates), the 0.5 degree file used in global simulation is available at: "~/data/nimpl_ISRIC/data_halfdeg/CNrt.nc" and "~/data/nimpl_sofun_inputs/Final_ncfile/CNrt.nc"
```{r}
library(raster)

soil <- raster('~/data/nimpl_ISRIC/data_orig/data/raster/w001000.adf') # map input from the last 2015 version of ISRIC (Batjes 2015 ISRIC report)

Global_grid <- as.data.frame(read.csv(file="~/data/nimpl_ISRIC/data_orig/data/Global_cell.csv")) #Input global land grids (0.5 degree;N = 259200)
...
ISRIC.data<-read.csv(file="~/data/nimpl_ISRIC/data_orig/data/HW30s_FULL.csv",header=TRUE,sep=";",dec = ".") # Now, input ISRIC database
...
#2. stand age
The original stand age file was available at "~/data/nimpl_GFAD/data_orig/GFAD_V1-1.nc", then after scripts below and after CDO processing (for correcting coordinates), the 0.5 degree file used in global simulation is available at: "~/data/nimpl_GFAD/data_halfdeg/age.nc" and "~/data/nimpl_sofun_inputs/Final_ncfile/age.nc"
```{r}
library(raster)
library(ncdf4)

#set the path and filename
ncfname <- paste ("~/data/nimpl_GFAD/data_orig/", "GFAD_V1-1", ".nc", sep="")
...
#On below, we do not calculate weight-mean, but calculate stand age from the maximum value at each grid.
...
i <- 1
for (i in (1:259200)){
  if (sum(pre.mat[i,57:60])>0){lonlat$age[i]<- 145} else { #here we extracted each grid from its maximum value
    if (sum(pre.mat[i,53:56])>0){lonlat$age[i]<- 135}else { #when any fraction data is available...
      if (sum(pre.mat[i,49:52])>0){lonlat$age[i]<- 125}else {
        if (sum(pre.mat[i,45:48])>0){lonlat$age[i]<- 115}else {
          if (sum(pre.mat[i,41:44])>0){lonlat$age[i]<- 105}else {
            if (sum(pre.mat[i,37:40])>0){lonlat$age[i]<- 95}else {
              if (sum(pre.mat[i,33:36])>0){lonlat$age[i]<- 85}else {
                if (sum(pre.mat[i,29:32])>0){lonlat$age[i]<- 75}else {
                  if (sum(pre.mat[i,25:28])>0){lonlat$age[i]<- 65}else {
                    if (sum(pre.mat[i,21:24])>0){lonlat$age[i]<- 55}else {
                      if (sum(pre.mat[i,17:20])>0){lonlat$age[i]<- 45}else {
                        if (sum(pre.mat[i,13:16])>0){lonlat$age[i]<- 35}else {
                          if (sum(pre.mat[i,9:12])>0){lonlat$age[i]<- 25}else {
                            if (sum(pre.mat[i,5:8])>0){lonlat$age[i]<- 15}else {
                              if (sum(pre.mat[i,1:4])>0){lonlat$age[i]<- 5}else {lonlat$age[i]<-NA
                              }}}}}}}}}}}}}}}}
...
#3. fAPAR
The original stand age file was available at "~/data/fAPAR/fAPAR3g_v2/", then after scripts below and after CDO processing (for correcting coordinates), the 0.5 degree file used in global simulation is available at: "~/data/nimpl_sofun_inputs/map/Final_ncfile/fAPAR.nc" and "~/data/nimpl_sofun_inputs/Final_ncfile/fAPAR.nc"
```{r}
library(ncdf4)
library(raster)
...
```
#4. alpha
The original stand age file was available at "~/data/alpha/data_orig/", then after scripts below and after CDO processing (for correcting coordinates), the 0.5 degree file used in global simulation is available at: "~/data/alpha/data_halfdeg/alpha.nc" and "~/data/nimpl_sofun_inputs/Final_ncfile/alpha.nc"
```{r}
library(raster)
...
The final nc file (0.5 degree file) after CDO processing was saved at "~/data/nimpl_sofun_inputs/Final_ncfile/PPFD.nc"
```{r}
library(raster)
library(ncdf4)
...
The final nc file (0.5 degree file) after CDO processing was saved at "~/data/nimpl_sofun_inputs/Final_ncfile/Tg.nc"
```{r}
library(raster)
library(ncdf4)
...
The final nc file (0.5 degree file) after CDO processing was saved at "~/data/nimpl_sofun_inputs/Final_ncfile/vpd.nc"
```{r}
library(raster)
library(ncdf4)
...
Here the all tiff. file was originally downloaded from TRY (see link on above). The original data of SLA was saved as "~/data/nimpl_TRY_maps/data_orig/SLA_3km_v1.tif". Then I have inputted this tif file into ArcGIS, and  interpolating all global grids points (720*360 grids) from the 3km tiff map, using "Extract multi values to points" function. This will enable us to output a global grid dataframe (in csv format), which exactly includes LMA values from 720*360 grids (i.e. N = 259200). The output csv is available in "~/data/nimpl_TRY_maps/SLA_export.csv". This csv was then processed at codes below (for correcting units), and then processed by a CDO function in Euler for correcting coordinates. After all this, the final nc file used for prediction field is now available at "~/data/nimpl_TRY_maps/data_halfdeg/LMA.nc" and "~/data/nimpl_sofun_inputs/map/Final_ncfile/LMA.nc"
```{r}
library(raster)
library(ncdf4)

SLA_extracted <- read.csv(file="~/data/nimpl_TRY_maps/SLA_export.csv") #This dataframe outputted from ArcGIS, including FID, lon, lat, Elevation, SLA (mg/mm)
...
#after this, we could output rLMA as a nc file using code below, and then after using CDO function in Euler, we will eventually generate final nc file used for global simulation.


``` 
#NC files generation
After finishing map at each vairable, we can directly outputing such object (e.g. rLMA, rvpd, rTg...) to nc file, using WriteRaster function in r. An example code outputnig rLMA is on below:
d <- writeRaster(rLMA, paste0("~/nimpl_sofun_inputs/map/Final_ncfile","LMA.nc"),
                 format = "CDF", overwrite = TRUE, varname = "LMA", 
                 varunit = "g/m2", longname = "Leaf-mass-per-area (TRY gloabl traits map)", 
                 xname = "lon", yname = "lat")
After this step, we need to have CDO processing so that flipped lat/lon problem would not occur. Then final nc files used in global simulation were all available at "~/data/nimpl_sofun_inputs/map/Final_ncfile/"