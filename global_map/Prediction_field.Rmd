...
#1. soil C/N
The original stand age file was available at "~/data/ISRIC/data_orig/", then after scripts below and after CDO processing (for correcting coordinates), the 0.5 degree file used in global simulation is available at: "~/data/ISRIC/data_halfdeg/CNrt.nc" and "~/data/sofun_inputs/Final_ncfile/CNrt.nc"
```{r}
library(raster)
...
Global_grid <- as.data.frame(read.csv(file="~/data/ISRIC/data_orig/data/Global_cell.csv")) #Input global land grids (0.5 degree;N = 259200)
...
#2. stand age
The original stand age file was available at "~/data/GFAD/data_orig/GFAD_V1-1.nc", then after scripts below and after CDO processing (for correcting coordinates), the 0.5 degree file used in global simulation is available at: "~/data/GFAD/data_halfdeg/age.nc" and "~/data/sofun_inputs/Final_ncfile/age.nc"
```{r}
library(raster)
library(ncdf4)
...
Here the all tiff. file was originally downloaded from TRY (see link on above). The original data of SLA was saved as "~/data/TRY_maps/data_orig/SLA_3km_v1.tif". Then I have inputted this tif file into ArcGIS, and  interpolating all global grids points (720*360 grids) from the 3km tiff map, using "Extract multi values to points" function. This will enable us to output a global grid dataframe (in csv format), which exactly includes LMA values from 720*360 grids (i.e. N = 259200). The output csv is available in "~/data/TRY_maps/SLA_export.csv". This csv was then processed at codes below (for correcting units), and then processed by a CDO function in Euler for correcting coordinates. After all this, the final nc file used for prediction field is now available at "~/data/TRY_maps/data_halfdeg/LMA.nc" and "~/data/nimpl_sofun_inputs/map/Final_ncfile/LMA.nc"
```{r}
library(raster)
library(ncdf4)
