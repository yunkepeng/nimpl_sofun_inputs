...
soil <- raster('~/data/nimpl_ISRIC/data/raster/w001000.adf') # map input from the last 2015 version of ISRIC (Batjes 2015 ISRIC report)

Global_grid <- as.data.frame(read.csv(file="~/data/nimpl_ISRIC/data/Global_cell.csv")) #Input global land grids (0.5 degree;N = 259200)
...
ISRIC.data<-read.csv(file="~/data/nimpl_ISRIC/data/HW30s_FULL.csv",header=TRUE,sep=";",dec = ".") # Now, input ISRIC database
...
ncfname <- paste ("~/data/nimpl_GFAD/", "GFAD_V1-1", ".nc", sep="")
...
```{r}
library(raster)
alphalist <- list.files(path = "~/data/alpha/",full.names = T) # here this file includes 116 data - that is the annual alpha from 1901 to 2016
