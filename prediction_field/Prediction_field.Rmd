...
#output age nc file. THIS IS NOT THE LAST STEP!!! We need to additionally apply CDO processing in Euler.
#d <- writeRaster(rage, paste0("D:/PhD/nimpl_sofun_inputs/Data/Final_ncfile/","age_0923_interpolated.nc"),format = "CDF", overwrite = TRUE, varname = "age", varunit = "yrs", longname = "stand-age",  xname = "lon", yname = "lat")
#Once cdo finished, the nc file below will be deleted, and output of CDO will be renamed to age.nc

#after CDO, have a check
#ncfname <- paste ("D:/PhD/nimpl_sofun_inputs/Data/Final_ncfile/", "age", ".nc", sep="")
#ncin <- nc_open(ncfname)
#lon <- ncvar_get(ncin,"lon")
#lat <- ncvar_get(ncin,"lat")
#head(lon)
...
#needs to convert NA to 9999, and then cdo processing, then can be input to nimpl
```
...
library(raster)
library(ncdf4)
ncfname <- paste ("~/data/nimpl_sofun_inputs/map/Final_ncfile/", "PPFD", ".nc", sep="")
...
library(raster)
library(ncdf4)
ncfname <- paste ("~/data/nimpl_sofun_inputs/map/Final_ncfile/", "Tg", ".nc", sep="")
...
library(raster)
library(ncdf4)
ncfname <- paste ("~/data/nimpl_sofun_inputs/map/Final_ncfile/", "vpd", ".nc", sep="")
...
Note that, due to LMA used by resampling method in R directly (see LMA_field.Rmd). This part (based after Arcgis resampling tool) was not in use, though still kept here.
...
#All final data saved as df_Tg, df_PPFD....which is consistent with final_ncfile/ (the one put in nimpl simulation) - although those dataframe (df_Tg...) needs two more steps, convert NA to 9999 (prevent FPE) and cdo processing
