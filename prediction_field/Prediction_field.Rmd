...
## read file as raster
raster_soil <- brick('~/data/ISRIC/data_orig/data/raster/w001000.adf')

## write as NetCDF
writeRaster(raster_soil, 
            "~/data/ISRIC/tmp_cnsoil.nc", 
            overwrite=TRUE, 
            format="CDF", 
            varname="cnsoil", 
            varunit="unit", 
            longname="Soil C:N ratio", 
            xname="lon", yname="lat"
            )

## regrid using CDO (see https://www.notion.so/computationales/CDO-3607611d56494dc6a566f4e40cd9f7cc)

## re-read CDO-aggregated and check if it's ok.

## if necessary fill missing values (e.g. knn: cn ~ lon + lat, k = 5)
...
writeRaster(raster_soil_agg, 
            "~/data/ISRIC/data_halfdeg/cn_soil_isric_halfdeg.nc", 
            overwrite=TRUE, 
            format="CDF", 
            varname="cnsoil", 
            varunit="unit", 
